...
with tab1:
    ...
    # [修正] 按鈕大小還原 (移除 use_container_width)
    if st.button("🚀 執行分析"):
        targets = []
        df_up = pd.DataFrame()
        ...
        if not df_up.empty:
            ...
            for col in df_up.columns:
                if "代號" in str(col): c_col = col; break
            n_col = None
            for col in df_up.columns:
                if "名稱" in str(col): n_col = col; break
